...
"""convert_dsd.py
   ...
   editing and converting PIP DSD data from .dat files 
...
##### Globals
data_path = '../example_data/LakeEffect/PIP/2020_MQT/PIP_3/f_1_4_DSD_Tables_ascii/'
...
def convert_dsd():
    ...
    for file in files:
        ...
        df.drop(columns=['Bin_cen', 'Num_d', 'day_time', 'hr_d', 'min_d', 'year', 'month', 'day', 'hour', 'minute'], inplace=True)
        ...
        ds['lat'] = 46.53
        ds['lon'] = -87.55
        ds['dsd'] =(['time', 'bin_centers'], df.to_numpy())
        ds['bin_edges'] = bin_edges
        
        ##### Define global/variable attributes according to CF-1.10 conventions
        ds.dsd.attrs['units'] = 'm−3 mm−1'
        ds.dsd.attrs['long_name'] = 'Drop size distributions'
        ds.dsd.attrs['standard_name'] = 'drop_size_distribution'
        ds.dsd.attrs['missing_value'] = 'NaN'
