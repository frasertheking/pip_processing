...
def open_eden_data(fdatname,year,month,day,loc):
    year = int(year)
    month = int(month) 
    day = int(day) 
    EDEN_avg = []
    ...
    if float(Bin_edge[0]) == 0.:
        ...
        EDEN_avg = EDEN_avg[5:]
    edenavg = []
    for binpoint in EDEN_avg:
        edenavg.append(float(binpoint))
    EDEN_avg = np.array(edenavg)
    
    #long list thing. should be the length of the minutes for which we have data
    eden_minute = dataread[4:]

    # the time miutes that we have data
    timestamped = []
    for element in eden_minute:
        timestamped.append(float(element[0]))
    ...
    if not timestamped:
        #print('there is no data on this day at all... but we still have a file. so here we are in this section')
        # let the first point in the day be the beginning spot
        edenpermin = np.zeros((np.shape(timealt)[0], np.shape(bin_edge)[0]))
        edenpermin.fill(-9999.)
        EDEN_avg = np.zeros((np.shape(bin_edge)))
        EDEN_avg.fill(-9999.)
    else:
        # this section deals with days that have no missing data or just a couple of missing points
        timestamped = np.array(timestamped)
        ...
        # fill in for missing data 
        zterm = np.zeros((1,np.shape(EDEN_minute)[1]))
        zterm.fill(-9999.)
        for i, intertime in enumerate(timealt):
            checkpoint = np.argwhere(np.array(uptime) == intertime)
            if len(checkpoint) >1:
                #print('uhoh: we have duplicate time stamps. using time variable from PSD file.')
                edenpermin.extend(zterm)
                """newcheckpoint1 = handle_duplicate_time_stamps(loc, intertime)
                distpermin.append(DIST_minute[newcheckpoint1])"""
            else:
                if checkpoint:
                    edenpermin.append(EDEN_minute[int(checkpoint)])
                if not checkpoint:
                    edenpermin.extend(zterm)
    
    edenperminute = np.array(edenpermin)
    edenperminute = numpy.ma.masked_less_equal(edenpermin,0.)
    EDEN_avg = numpy.ma.masked_less_equal(EDEN_avg,0.)
    return EDEN_avg, edenperminute, bin_cen,bin_edge, timevariable, instrument_time 

def open_vvd_data(fdatname,year,month,day,loc):
    year = int(year)
    month = int(month) 
    day = int(day) 
    vvd_avg = []
    ...
    if float(Bin_edge[0]) == 0.:
        ...
        vvd_avg = vvd_avg[5:]
    vvdavg = []
    for binpoint in vvd_avg:
        vvdavg.append(float(binpoint))
    vvd_avg = np.array(vvdavg)
    
    #long list thing. should be the length of the minutes for which we have data
    vvd_minute = dataread[4:]

    # the time miutes that we have data
    timestamped = []
    for element in vvd_minute:
        timestamped.append(float(element[0]))
    ...
    if len(missing)>= 2:
        for missingpoint in missing:
            ...
            del(timestamped[missingpoint])
            #remove the same potentially missing elements from the DSD_minute var as well
            del(vvd_minute[missingpoint])
    else:
        #remove potentially missing elements from the timevar
        del(timestamped[int(missing)])
        #remove the same potentially missing elements from the DSD_minute var as well
        del(vvd_minute[int(missing)])
    ...
    if not timestamped:
        #print('there is no data on this day at all... but we still have a file. so here we are in this section')
        # let the first point in the day be the beginning spot
        vvdpermin = np.zeros((np.shape(timealt)[0], np.shape(bin_edge)[0]))
        vvdpermin.fill(-9999.)
        vvd_avg = np.zeros((np.shape(bin_edge)))
        vvd_avg.fill(-9999.)
    else:
        # this section deals with days that have no missing data or just a couple of missing points
        timestamped = np.array(timestamped)
        ...
        # fill in for missing data 
        #zterm = np.zeros((1,np.shape(vvd_minute)[1]))
        #zterm.fill(-9999.)
        #for i, intertime in enumerate(timealt):
            #checkpoint = np.argwhere(np.array(uptime) == intertime)
            #if len(checkpoint) >1:
                #print('uhoh: we have duplicate time stamps. using time variable from PSD file.')
                #vvdpermin.extend(zterm)
                #"""newcheckpoint1 = handle_duplicate_time_stamps(loc, intertime)
                #distpermin.append(DIST_minute[newcheckpoint1])"""
            #else:
                #if checkpoint:
                   # vvdpermin.append(vvd_minute[int(checkpoint)])
                #if not checkpoint:
                    #vvdpermin.extend(zterm)
    
    vvdperminute = np.array(vvdpermin)
    vvdperminute = numpy.ma.masked_less_equal(vvdpermin,0.)
    vvd_avg = numpy.ma.masked_less_equal(vvd_avg,0.)
    return vvd_avg, vvdperminute, bin_cen,bin_edge, timevariable, instrument_time 
