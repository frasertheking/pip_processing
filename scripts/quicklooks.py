...
def create_hists_for_site(site):
    ...
    axes[0].imshow(ze_hist, origin='lower', cmap='Reds', aspect='auto', norm=LogNorm(vmin=0.001, vmax=np.nanmax(ze_hist)), extent=[ze_yedges[0], ze_yedges[-1], ze_xedges[0], ze_xedges[-1]])
    axes[0].invert_yaxis()
    ...
    axes[1].imshow(dv_hist, origin='lower', cmap='Blues', aspect='auto', norm=LogNorm(vmin=0.001, vmax=np.nanmax(dv_hist)), extent=[dv_yedges[0], dv_yedges[-1], dv_xedges[0], dv_xedges[-1]])
    axes[1].invert_yaxis()
    axes[1].set_xlabel("m s$^{-1}$")
    axes[1].set_xlim((-10, 10))
    axes[2].set_title("Spectral Width")
    axes[2].imshow(sw_hist, origin='lower', cmap='Oranges', aspect='auto', norm=LogNorm(vmin=0.001, vmax=np.nanmax(sw_hist)), extent=[sw_yedges[0], sw_yedges[-1], sw_xedges[0], sw_xedges[-1]])
    axes[2].invert_yaxis()
    ...
    axes[0].imshow(dsd_hist.T, origin='lower', cmap='viridis', aspect='auto', norm=LogNorm(vmin=0.01, vmax=np.nanmax(dsd_hist)), extent=[dsd_xedges[0], dsd_xedges[-1], dsd_yedges[0], dsd_yedges[-1]])
...
#     axes[0].set_ylim((0, 1000))
    ...
    axes[1].imshow(vvd_hist.T, origin='lower', cmap='Purples', aspect='auto',  norm=LogNorm(vmin=0.01, vmax=np.nanmax(vvd_hist)), extent=[vvd_xedges[0], vvd_xedges[-1], vvd_yedges[0], vvd_yedges[-1]])
    ...
    axes[2].imshow(rho_hist.T, origin='lower', cmap='Greens', aspect='auto',  norm=LogNorm(vmin=0.01, vmax=np.nanmax(rho_hist)),extent=[rho_xedges[0], rho_xedges[-1], rho_yedges[0], rho_yedges[-1]])
