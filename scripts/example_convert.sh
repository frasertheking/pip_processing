...
mkdir -p "${OUT_PATH}dsd/"
mkdir -p "${OUT_PATH}vvd/"
mkdir -p "${OUT_PATH}rho/"
mkdir -p "${OUT_PATH}ed/"
...
do
    ...
    for file in "${DATA_PATH}${i}"*"${wild[$LOC]}".dat; do
        ...
        if [[ $LOC -lt 3 ]]; then
            python dist_wrap.py $file "${OUT_PATH}${vars[$LOC]}/" "${vars[$LOC]}" $LAT $LON "${units[$LOC]}" "${long[$LOC]}" "${standard[$LOC]}"
        else
